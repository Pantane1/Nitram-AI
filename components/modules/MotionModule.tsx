  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      if (window.aistudio?.hasSelectedApiKey) {
        ...
        setHasKey(true); // Fallback for standard environments
      }
    };
    checkKey();
  }, []);
      ...
      <div className="h-full flex items-center justify-center p-6 text-center">
        <div className="max-w-md space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-neutral-400 text-sm">Video generation requires a specialized API key from a paid GCP project.</p>
          </div>
          <button 
    ...
    <div className="h-full flex gap-6 p-6 overflow-hidden">
      <div className="w-80 flex flex-col gap-6 shrink-0">
        ...
        <div className="space-y-3 mt-auto">
          ...
          >
            {loading ? 'Rendering...' : 'Create Motion Asset'}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-neutral-900 rounded-2xl border border-neutral-800 flex items-center justify-center relative overflow-hidden group">
        {result ? (
          <video src={result} controls autoPlay loop className="max-w-full max-h-full rounded-lg shadow-2xl border border-neutral-700" />
        ) : (
